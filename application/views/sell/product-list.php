    ...
    <div class="col-sm-12">
        <div class="panel panel-default thumbnail">
            <div class="panel-heading no-print col-md-12" style="padding-top:15px;">
                <form action="<?=base_url('buy');?>" method="get">
                    <div class="col-md-8">
                        <input type="text" name="searched_product"
                            value="<?php if(isset($_GET['searched_product'])){ echo $_GET['searched_product'];} ?>"
                            class="form-control" placeholder="Enter Keywords..."><br>
                ...
                <div class="col-md-2">
                    <a class=" btn btn-success col-xs-12 col-sm-12" href="<?php echo base_url("buy"); ?>"> Items :
                        <?php echo $totalprod; ?></a>
                </div>
				...
				if(!empty($product_list)) {
                    ...
                    <div class="col-md-3 ">
                        <div class="card hover" style="position:relative;display:block;">
                            <div class="box20">
                                <img src="<?php  echo base_url("assets/images/products/".$prd->image); ?>"
                                ...
                                <ul class="icon">
                                    <li>
                                        ...
                                        <a href="javascript:void(0)" class="minus-quantity"
                                            data-prodid="<?php echo $prd->id; ?>"
                                            data-minalert="<?php echo $prd->minimum_order_quantity ?>"><span> -
                                            </span></a>
                                        <?php } ?>

                                        <a href="javascript:void(0)" data-prodid="<?php echo $prd->id; ?>"
                                            class="add-to-cart"
                                            data-minalert="<?php echo $prd->minimum_order_quantity ?>">
                                            <?php
									$plus = "shopping-cart";	
                                    ...
                                    <li>
                                        <a href="<?php echo base_url("buy/view/".$prd->id); ?>"> <i class="fa fa-eye"
					...
					?> <ul class="pagination">
						...
						if($page > 1) { ?>
                                <li class="page-item"><a class="page-link"
                                        href="<?php echo base_url("buy?page=".($page - 1)).$nurl.''; ?>">Previous</a>
                                </li>
                                <?php
						}
			...
			?>
                                <li class="page-item"><a class="page-link"
                                        href="<?php echo base_url("buy?page=".($page + 1)).$nurl.''; ?>">Next</a></li>
                                <?php }
			}	
			?>
...
function cartchange(currobj, qty) {
    ...
    $.ajax({
        ...
        type: "post",
        data: {
            product: currobj.data("prodid"),
            qty: qty,
            "minimum": crtobj.data("minalert")
        },
        success: function(resp) {
            ...
            if (jresp.status == 1) {
                ...
                currobj.closest("li").prepend(
                    '<a href = "javascript:void(0)" class = "minus-quantity" data-prodid = "' + jresp
                    .prodid + '"><span> - </span></a>');
                var tprice = parseInt(jresp.price) * parseInt(jresp.qty);
                var htmcnt = '<li id = "cart-li-' + jresp.prodid +
                    '"><div class="cart-items"><h4><a href="" class="prodname">' + jresp.product +
                    ' </a></h4>' +
                    '<p><a href=""> Price : <i class="fa fa-rupee"></i> ' + jresp.price + ' X ' +
                    '<input type="number" value="' + jresp.qty + '" min=1 class="cart-qty" data-prodid=' +
                    jresp.prodid + '><input type="hidden" name="minimum" class="minimum" value="' + jresp
                    .minalert + '">' + ' = <i class="fa fa-rupee"></i> <span class="item-price-' + jresp
                    .prodid + '">' + tprice + '</span> </a>' +
                    '</p></div><hr /></li>';
                ...
                if (parseInt(jresp.qty) > 0) {
                    ...
                    crtobj.find(".cart-quantity").text(parseInt(crtobj.find(".cart-quantity").text()) +
                    qty);

                    var tprice = parseInt(jresp.price) * parseInt(jresp.qty);
                    $("#cart-li-" + jresp.prodid).html(
                        '<div class="cart-items"><h4><a class="prodname" href="">' + jresp.product +
                        ' </a></h4>' +
                        '<p><a href=""> Price : <i class="fa fa-rupee"></i> ' + jresp.price + ' X ' +
                        '<input type="number" value="' + jresp.qty +
                        '" min=1 class="cart-qty" data-prodid=' + jresp.prodid +
                        '><input type="hidden" name="minimum" class="minimum" value="' + jresp
                        .minalert + '">' + ' = <i class="fa fa-rupee"></i> <span class="item-price-' +
                        jresp.prodid + '">' + tprice + '</span> </a>' +
                        '</p></div>');
                } else {
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            if (jresp.status == 1) {
                ...
                currobj.append('<span class="cart-quantity">1</span>');
                currobj.closest("li").prepend(
                    '<a href = "javascript:void(0)" class = "minus-quantity" data-prodid = "' +
                    jresp.prodid + '"><span> - </span></a>');
                var tprice = parseInt(jresp.price) * parseInt(jresp.qty);
                var htmcnt = '<li id = "cart-li-' + jresp.prodid +
                    '"><div class="cart-items"><h4><a href="" class="prodname">' + jresp.product +
                    ' </a></h4>' +
                    '<p> Price : <i class="fa fa-rupee"></i> ' + jresp.price + ' X ' +
                    '<input type="number" value="' + jresp.qty +
                    '" min=1 class="cart-qty" data-prodid=' + jresp.prodid +
                    '><input type="hidden" name="minimum" class="minimum" value="' + jresp
                    .minalert + '">' + ' = <i class="fa fa-rupee"></i> <span class="item-price-' +
                    jresp.prodid + '">' + tprice + '</span>' +
                    '<a href="javascript:void(0)" onclick="remove_cart_item(' + jresp.prodid +
                    ')" class="fa fa-trash btn btn-danger btn-sm pull-right remove-item-cart"></a></p></div><hr /></li>';
                $("#cart-nav-menu").prepend(htmcnt);
                $("#nav-cart-count").text(jresp.total);
                try {
                    manageCartQty();
                ...
                currobj.find(".cart-quantity").text(parseInt(currobj.find(".cart-quantity")
                .text()) + 1);
                ...
                $("#cart-li-" + jresp.prodid).html(
                    '<div class="cart-items"><h4><a href="" class="prodname">' + jresp.product +
                    ' </a></h4>' +
                    '<p><a href=""> Price : <i class="fa fa-rupee"></i> ' + jresp.price +
                    ' X ' + '<input type="number" value="' + jresp.qty +
                    '" min=1 class="cart-qty" data-prodid=' + jresp.prodid +
                    '><input type="hidden" name="minimum" class="minimum" value="' + jresp
                    .minalert + '">' +
                    ' = <i class="fa fa-rupee"></i> <span class="item-price-' + jresp.prodid +
                    '">' + tprice + '</span> </a>' +
                    '<a href="javascript:void(0)" onclick="remove_cart_item(' + jresp.prodid +
                    ')" class="fa fa-trash btn btn-danger btn-sm pull-right remove-item-cart"></a></p></div>'
                    );
            }

  <div style="width:70%;float:right;">
	  <style>
	  .class_chat_list{position:fixed;z-index:200;float:right;right:0px;bottom:0px;display:block;background:#fff;height:416px;width:290px;
    ...
    box-shadow: 0 0 7px rgba(0,0,0,0.2);
	  ...
	  #chatSection,.chatSection{display:inline-block;float:right;background:#fff;height:488px;width:318px;margin-right: 10px;
	  border:1px solid #dadada;    border-radius: 10px 10px 0px 0px;
   /* overflow: hidden; */
-webkit-box-shadow: 0 0 7px rgba(0,0,0,0.2);
-moz-box-shadow: 0 0 7px rgba(0,0,0,0.2);
    box-shadow: 0 0 7px rgba(0,0,0,0.2);
...
<style>
	...
	.remove-chat-window,.close-login-user{
		    display: block;
    float: right;
    margin-right: 12px;
	...
	.remove-chat-window  .btn, .ui-datepicker-buttonpane button, .sp-container button{
		    line-height: 1;
			    color: #bababa;
	}
	.img-show{
	...
	.emoji-txt-icons .fa,.view-emoji-icon {
		font-size: 32px;
    color: #adaf2b;
	}
	.tb-emoji-icon {
...
.d-flex {
    display: flex !important; 
	   ...
	   <div class = "minimize-chatbox" style="position:fixed;z-index:200;float:right;right:70px;bottom:0px;display:block;" id="maxmize_chates">
		...
		<div id = "chatbox-container"><div id = "all-chats-area"></div></div>	
            ...
            <div class="chatSection" id="chatSection" style="display:none">
              ...
              <div class="box box-warning direct-chat direct-chat-primary">
                <div class="box-header with-border" style="height:40px;border-bottom: 1px solid #dadada;">
                  ...
                  <span class="box-title ReciverName_txt" style="color:#484848;font-weight:900;margin-left:10px;margin-top:8px;line-height:40px;" id="ReciverName_txt"></span>
				  
				   <a href= "javascript:void(0)" class = "btn btn-default remove-chat-window"  onclick="closed_comment();"> X
                ...
                <div class="box-footer">
                    <div class="input-group">
                        ...
                        <div  placeholder="Type Message press enter..." class="form-control message" style="width:70%"  contenteditable="true"></div>

						<div class="dropup">
						  <a class="btn btn-emoji dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            ...
            <div class="class_chat_list" id="user_list_hide" style="display:none">
              ...
              <div class="box box-danger">
                <div style="background:#fff !important;">
                  ...
                  <h4 style="color: #484848; font-weight: 900; margin-left: 10px; margin-top: 8px;line-height: 34px;font-size: 14px;">User List   <a style="font-size:16px;margin-top:2px;" href="javascript:void(0)" class="btn btn-default close-login-user btn-small"  onclick="closed_list();"> X
                    </a></h3>
                  </div>
                ...
                <div >
                  <div class="users-list" id = "chat-user-box" style="height:375px;overflow-y:auto;margin-top:-10px;padding:10px;">
                    <?php 
					 ...
					 if(!empty($all_user)){
						...
						?>
						<div style="border-bottom:2px solid #eee;height:50px;padding-top:5px;" id="<?=$v->pk_i_admin_id;?>" title="<?=$v->s_display_name.''.$v->last_name;?>" class="selectuser">
                      
						<div style="float:left;width:15%;height:50px;">
						 <?php if($v->picture){?>
                          <img style="height:40px;width:40px;border-radius:50%;" src="<?=base_url().$v->picture;?>" title="<?=$v->s_display_name.''.$v->last_name;?>">
                         <?php }else{?>
						  <img style="height:40px;width:40px;border-radius:50%;" src="<?php echo base_url();?>uploads/user-icon.jpg" title="<?=$v->s_display_name.''.$v->last_name;?>">
						 <?php } ?>
						 </div>
						 <div style="float:right;width:75%;height:50px;">
						 <span style="font-size:12px;"><?=$v->s_display_name.''.$v->last_name;?> <span  class="total-unread-msg"></span>
						 ...
						 <span class ="user-status-point"  style="font-size:10px;float:right;border:5px solid <?php echo $clr; ?>;border-radius:50%"></span><br></span><span style="font-size:10px;"><?=$v->user_role;?></span><span class="prev-login-user" style="font-size:10px;float:right;">
						 <?php $lstlog = $v->last_log; 
...
function GetChatHistory(receiver_id, othr){
		...
		if(othr  == "1"){
			...
			field = "&action=read";
				...
				$.ajax({
						  //dataType : "json",
  						  url: '<?php echo base_url();?>ChatController/get_chat_history_by_vendor?receiver_id='+receiver_id+field,
						  success:function(data)
						  {
...
function makehtml(user){
	...
	if(user.type == "1"){
		...
		var msgcnt  = '<div class="'+cntcls+'">'+user.msgbody+'<span class="msg_time">'+user.msgdate+'</span></div>';
			...
			var jresp  = JSON.parse(data);
				if(parseInt(jresp.total) > 0){
					...
					$.each(jresp.users, function(ind, jarr){
						...
						$("#"+ind).find(".user-status-point").addClass("log-sts-"+jarr.color);
						...
						if(parseInt(jarr.unread) > 0 ) {
							...
							$("#"+ind).find(".total-unread-msg").addClass("badge badge-success").text(jarr.unread + " New");
						}else{
							$("#"+ind).find(".total-unread-msg").removeClass("badge badge-success").text('');
						}
			...
			success:function(data)
			{
				...
				if(jresp.total > 0){
					...
					$.each(jresp.users, function(ind, msg){
						...
						$("#"+ind).find(".total-unread-msg").addClass("badge badge-success").text(msg + " New");
					});
				}

	...
	public function career_ex_contact_form_post(){
		...
		if ($this->form_validation->run() == true) {	
			...
			curl_setopt_array($curl, array(
			  	...
			  	CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
			  	CURLOPT_CUSTOMREQUEST => "POST",
			  	CURLOPT_POSTFIELDS => array('fname' => $fname,'lastname' => $last_name,'email' => $email,'mobileno' => $mobile,'company_id' => '81','process_id' => 175,'product_id' => $product_id,'enquiry'=>$join_date,'user_id' => '511','address'=>$address,'expected-joining-date'=>$join_date),
			  	CURLOPT_HTTPHEADER => array(
			    	"Cookie: ci_session=3ba7d4lq4alv2pgpq3sc8t2ojrh41s04"
			  	),
			));
